...
void HtmlTags::defineInitialSymbol(const StringA& htmlText, LInt htmlType)
{
	m_map.set(StringUtils::hashCode(htmlText), htmlType);
}
...
HtmlTags* HtmlTags::getInstance()
{
	if (m_htmlTags == NULL)
	{
		...
		m_htmlTags->defineInitialSymbol(_CS("hr"), HR); // 34
...
int HtmlTags::symbolAsInt(util::StringA& inSymbol)
{
