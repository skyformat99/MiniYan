...
void RenderContext::newLine(HtmlItem* item)
{
	if (m_center == LTrue)
	{
		relayoutLineItems();
    ...
    HtmlItem* containerItem = item->getContainingBlock();
    if (containerItem)
    {
        m_x  = containerItem->getStyle()->paddingLeft();
    }
    else
    {
        m_x  = m_newLineXStart;           
...
void RenderContext::addLineItem(HtmlItem* item)
{
	if (m_center == LTrue)
	{
...
void RenderContext::relayoutLineItems()
{
	...
	HtmlItemList::Iterator iter = m_lineItems.begin();
	HtmlItemList::Iterator iterEnd = m_lineItems.end();
	HtmlItemList::Iterator iterFinal = iterEnd;
	iterFinal--;
