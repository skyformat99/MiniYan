		...
		IItemListener* itemListener)
{
	...
	if (NULL == m_tinyDoc)
	{
	    m_tinyDoc = new TiXmlDocument();
	}

	m_tinyDoc->Parse((const LCharA*)buffer.GetBuffer(), 0, TIXML_ENCODING_UTF8);
	TiXmlElement* root = m_tinyDoc->RootElement();
...
void TinyDom::createRenderTree(TiXmlElement* elem, TiXmlElement* parentElem, HtmlItem* parent)
{
	...
	{
	    createRenderTree((TiXmlElement*)child, elem, item);
	}
}

HtmlItem* TinyDom::createHtmlItem(TiXmlElement* elem, TiXmlElement* parentElem, HtmlItem* parent)
{
	HtmlItem* item = NULL;
	HtmlTags* htmlTags = HtmlTags::getInstance();

	if (elem->Type() != TiXmlNode::TINYXML_TEXT && elem->Type() != TiXmlNode::TINYXML_COMMENT)
	{
	    StringA tagType = _CS("");
	    StringA tagId = _CS("");
	    StringA tagName = _CS("");
		...
		case HtmlTags::IMG:
			{
				...
				KFORMATLOG("HtmlTags::IMG src=%s", src);
				if (NULL != src)
				{
...
//			break;
	...
	else if (elem->Type() == TiXmlNode::TINYXML_TEXT)// text
	{
		if (NULL != parentElem)
		{
