...
MiniClientSocket::MiniClientSocket(MiniClientListener* listener)
    : m_socketHandle(-1)
    , m_listener(listener)
{
}

MiniClientSocket::~MiniClientSocket()
{
	closeSocket();
}

int MiniClientSocket::initClientSocket(const CString& hostName, int serverPort)
{
	struct hostent *host;
...
int MiniClientSocket::requestData(const CString& data)
{
...
int MiniClientSocket::receiveData()
{
	int recvLength = -1;
	do
	{
...
void MiniClientSocket::closeSocket()
{
	// 客户端正常结束
	if (m_socketHandle != -1)
	{
...
void MiniClientSocket::blockClose()
{
	AutoLock lock(&m_mutex);
	closeSocket();
}
