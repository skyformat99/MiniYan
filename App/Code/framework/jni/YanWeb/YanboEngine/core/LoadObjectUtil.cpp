...
float atofUtil(const StringA& s)
{
...
void LoadObjectUtil::loadFromFileVertexOnly(const CString& fname, float rz,float gz,float bz)
{
	...
	{
	    // 用空格分割行中的各个组成部分
	    KVector<StringA>* tempsa = util::StringUtils::split(buffer, _CS(" "));
	    ...
	    else if (tempsa->elementAt(0) == _CS("f"))
	    {
	         ...
	         *提取相应的顶点坐标值添加到结果顶点坐标列表中，同时根据三个
	      	...
	      	*/
	      	...
	      	KVector<StringA>* tempsaList1 = util::StringUtils::split(tempsa->elementAt(1), _CS("/"));
	      	...
	      	KVector<StringA>* tempsaList2 = util::StringUtils::split(tempsa->elementAt(2), _CS("/"));
	      	...
	      	KVector<StringA>* tempsaList3 = util::StringUtils::split(tempsa->elementAt(3), _CS("/"));
